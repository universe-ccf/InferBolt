...
from core.pipeline import voice_sentence_loop
...
def on_user_submit_audio_stream(audio_tuple, session_state, role_name, llm_client):
    """
    audio_tuple: (sr, np.ndarray) 或 gr.Audio 返回的 filepath
    这里假设还是拿到 (sr, samples)；若是 filepath，先读成 np，再送入 ASR。
    """

    sr, audio_np = audio_tuple  # 如果你拿到的是 filepath，改成读取 wav -> np
    ...
    ui_msgs = []
    # 接生成器
    for step in voice_sentence_loop(audio_np=audio_np, sample_rate=sr, state=session_state, role=role, llm_client=llm_client, asr_client=asr, tts_client=tts):
        # 累加要刷到 Chatbot 的消息
        ...
        # 每次yield更新：Chatbot、SessionState、Audio、Status
        yield ui_msgs, session_state, step.get("audio_path"), step.get("status", "")
