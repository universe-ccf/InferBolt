...
def build_ui():
    with gr.Blocks(title="Voicery · 思辨训练营", theme=THEME, css=CUSTOM_CSS) as demo:
        ...
        with gr.Row(elem_id="input_row"):
            ...
            with gr.Column(scale=1):
                with gr.Row():
                    re_record_btn = gr.Button("🔁 录制", variant="secondary", elem_id="mic_btn")
                with gr.Row():
        ...
        mic_clear_flag = gr.State(False)   # 标记这次 None 是“我自己清”的
        ...
        mic.change(
            ...
            lambda: None,     # 把 mic 的值置空
            None,
            mic               # 输出目标是 mic 组件
        ) # 每一轮“录→发→播”后，麦克风都会恢复到“空白可重新录”的状态
