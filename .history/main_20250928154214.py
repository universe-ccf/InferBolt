...
def on_user_submit_text_stream(user_text: str,
    ...
    try:
        role = load_role_config(role_name)
        ui_msgs = list(chatbot_hist or [])
        ui_msgs.append((user_text, ""))   # 占位
        yield ui_msgs, "—", "—", session

        # 用 respond() 取得最终文本（服务端已SSE清洗），如果你想要更“细粒度”，
        # 可在 LLMClient 里再加 complete_chunks() 返回片段，这里改为逐片拉。
        ...
        import traceback
        traceback.print_exc()
        (chatbot_hist or []).append((user_text, "抱歉，内部错误。"))
