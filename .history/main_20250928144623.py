...
def on_user_submit_text_stream(user_text: str,
                               chatbot_cur: list,          # 当前 Chatbot 的历史
                               session: SessionState,
                               role_name: str,
                               llm: LLMClient,
                               debug_on: bool):
    """
    生成器：在“已有对话”的基础上追加一轮，并分片刷新 assistant 文本。
    """
    try:
        role = load_role_config(role_name)
        # 复制一份当前对话（避免原列表原地被 Gradio 共享）
        ui_msgs = list(chatbot_cur or [])
        # 先占位一条空消息
        ui_msgs.append((user_text, ""))
        yield ui_msgs, "—", "—", session

        reply_full = respond(user_text=user_text, state=session, role=role, llm_client=llm).reply_text

        buf = []
        for ch in reply_full:
            buf.append(ch)
            if len(buf) >= 6:
        ...
        import traceback
        traceback.print_exc()
        # 出错时也把这一轮追加为错误文本，而不是覆盖历史
        ui_msgs = list(chatbot_cur or [])
        ui_msgs.append((user_text, "抱歉，内部错误。"))
        yield ui_msgs, "—", "—", session
...
def on_user_submit_audio_stream(audio_tuple,
    ...
    for step in gen:
        ...
        yield ui_msgs, step.get("audio_path"), step.get("status", ""), step.get("skill_label", "—"), session
    
    # for step in gen:
    #     # 把 step.chat_add 合入 Chatbot（去重：同一角色同一句只落一次）
    #     for who, txt in step.get("chat_add", []):
    #         if who == "user":
    #             # 若最后一条就是用户占位（assistant 为空），就原地覆盖文本
    #             if ui_msgs and ui_msgs[-1][0] is not None and ui_msgs[-1][1] is None:
    #                 ui_msgs[-1] = (txt, None)
    #             else:
    #                 ui_msgs.append((txt, None))
    #         elif who == "assistant":
    #             # 若最后一条是用户占位（assistant 为空），补齐 assistant
    #             if ui_msgs and ui_msgs[-1][0] is not None and ui_msgs[-1][1] is None:
    #                 ui_msgs[-1] = (ui_msgs[-1][0], txt)
    #             else:
    #                 # 否则追加一条新的 assistant（避免重复）
    #                 if not (ui_msgs and ui_msgs[-1][0] is None and ui_msgs[-1][1] == txt):
    #                     ui_msgs.append((None, txt))


    #     # 逐次刷新：Chatbot、Audio(单句路径)、状态徽标文本、技能徽标文本、Session
    #     skill_txt = step.get("skill_label", "—")  # 若还没接 classify，可先返回 “—”
    #     html = f'<audio autoplay controls src="{step.get("audio_path","")}"></audio>'
    #     yield ui_msgs, html, step.get("status",""), skill_txt, session
...
def build_ui():
    with gr.Blocks(title="Voicery · 思辨训练营", theme=THEME, css=CUSTOM_CSS) as demo:
        ...
        audio_out = gr.HTML(value="", visible=True)
        ...
        send_btn.click(
            ...
            inputs=[txt_in, chatbot, session_state, role_dd, llm_client, debug_ck],
