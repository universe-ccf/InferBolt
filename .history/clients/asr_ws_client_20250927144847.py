...
class ASRWsClient:
    ...
    async def _transcribe_async(self, audio_np: np.ndarray, sample_rate: int,
        ...
        try:
            async def _open_ws(self, headers_list):
                """
                兼容不同 websockets 版本的连接器：
                - 优先 extra_headers
                - 退到 additional_headers
                - 最后无 headers（会 401，但至少能明确地报错）
                """
                kw = dict(max_size=100_000_000, open_timeout=10)
                # 1) extra_headers
                try:
                    return await websockets.connect(self.ws_url, extra_headers=headers_list, **kw)
                except TypeError:
                    pass
                # 2) additional_headers
                try:
                    return await websockets.connect(self.ws_url, additional_headers=headers_list, **kw)
                except TypeError:
                    pass
                # 3) no headers（大概率 401）
                return await websockets.connect(self.ws_url, **kw)
