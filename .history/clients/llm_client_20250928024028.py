...
class LLMClient:
    ...
    def complete(self, messages, max_tokens=512, temperature=None, stream=False, timeout=None):
        """
        与调用方保持兼容：支持 stream 参数（当前默认 false），以免 TypeError。
        七牛 openai 兼容接口不要求必须包含 stream 字段，传 False 安全。
        """
        ...
        payload = {
            ...
            "stream": bool(stream),                # 接收上层 stream 参数，但默认 False；服务端一般也支持 False
        ...
        # 请求与容错（带重试可以沿用你之前的 session；这里用最简单 requests.post）
        req_timeout = timeout or getattr(settings, "REQUEST_TIMEOUT", 30)
        try:
            resp = self.session.post(url, headers=headers, json=payload, timeout=req_timeout) if hasattr(self, "session") else requests.post(url, headers=headers, json=payload, timeout=req_timeout)
            ...
            raise RuntimeError(f"LLM HTTP error: {getattr(e.response, 'text', str(e))[:400]}")
