...
def respond_short(user_text: str, state: SessionState, role: RoleConfig, llm_client) -> TurnResult:
    ...
    msgs = assemble_messages(system_text=sys_prompt, history=history, user_text=user_text, max_rounds=3)
    msgs = assemble_messages(system_text=sys_prompt, history=state.history, user_text=user_text, max_rounds=3)
    # 也可在 user 侧再加一句“请简洁回答”
    reply = llm_client.complete(msgs, max_tokens=256)
    # 更新会话
    state.add_turn(user=user_text, assistant=reply)
...
def voice_sentence_loop(audio_np, sample_rate, state: SessionState, role: RoleConfig, llm_client, asr_client, tts_client) -> Generator[Dict[str, Any], None, None]:
    ...
    yield {"status": f"🎧 已识别：{user_text_all}（分{len(sentences)}句处理）", "audio_path": None, "user_text": user_text_all, "chat_add": [("user", user_text_all)]}
    ...
    for idx, sent in enumerate(sentences, 1):
        ...
        # 3.3 TTS（单句）
        tts = tts_client.synthesize(turn.reply_text, voice_type=(getattr(role, "tts", {}) or {}).get("voice_type"), speed_ratio=(getattr(role, "tts", {}) or {}).get("speed_ratio"))
        # 3.4 逐句推送
        yield {
            "status": f"🗣️ 第{idx}/{len(sentences)}句：{sent}",
            "audio_path": tts.audio_path,   # gr.Audio 可直接播
            "user_text": sent,
