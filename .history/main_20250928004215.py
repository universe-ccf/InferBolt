...
def _load_voices():
    try:
        tts = TTSClient()
        items = tts.list_voices()  # [{voice_name, voice_type, ...}]
        labels, mapping = [], {}
        for it in items:
...
def build_ui():
    with gr.Blocks(title="Voicery · 思辨训练营", theme=THEME, css=CUSTOM_CSS) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2, visible=False, elem_id="drawer") as drawer:
                ...
                with gr.Group(elem_id="right_card"):
                    ...
                    voice_label_dd   = gr.Dropdown(label="音色（从官方列表加载）", choices=[], value=None)
        ...
        # 高级设置：开/合 + 拉取音色
        settings_btn.click(
            _toggle_drawer, inputs=[drawer], outputs=[drawer]
        ).then(
