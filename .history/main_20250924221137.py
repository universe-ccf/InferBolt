...
def build_ui():
    with gr.Blocks(title="AI 角色扮演 · MVP") as demo:
        ...
        llm_client = gr.State(LLMClient(model="dummy"))
        ...
        with gr.Tab("文本对话"):
            with gr.Row():
                txt_in = gr.Textbox(label="输入你的话", placeholder="先用文本跑通闭环…", lines=2)
...
def build_ui():
    with gr.Blocks(title="AI 角色扮演 · 文本闭环(MVP)") as demo:
        ...
        chatbot = gr.Chatbot(label="对话区", height=380)
